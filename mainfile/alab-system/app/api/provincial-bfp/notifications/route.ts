...
import { bfpNotificationUser } from "../../../../lib/notifications/session";

export const runtime = "nodejs";

export async function GET(request: NextRequest) {
  const identity = await bfpNotificationUser(request, "PROVINCIAL_BFP");
  if (!identity) return NextResponse.json({ error: "Provincial BFP sign-in is required." }, { status: 401 });
  try {
    ...
    console.error("Provincial notification lookup failed", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    console.error("Provincial notification update failed", error);
