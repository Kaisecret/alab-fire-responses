...
export async function GET(request: NextRequest) {
  const identity = await bfpNotificationUser(request, "MUNICIPAL_BFP");
  if (!identity) return NextResponse.json({ error: "Municipal BFP sign-in is required." }, { status: 401 });
  try {
    ...
    console.error("Municipal notification lookup failed", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    console.error("Municipal notification update failed", error);
