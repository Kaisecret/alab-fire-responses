...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const incident = await withTransaction(async (client) => {
      const current = await client.query<{ status: FireReportStatus }>("select status from fire_reports where id = $1 and municipality_id = $2 for update", [id, identity.municipalityId]);
