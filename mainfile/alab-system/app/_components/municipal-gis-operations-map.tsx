...
export function MunicipalGisOperationsMap() {
  const { municipality, incidents, loading, checking, refreshing, error, lastCheckedAt, refresh } = useMunicipalIncidentFeed({ includeHistory: true });
  ...
  const updatedLabel = checking ? "Refreshing municipal reports…" : lastCheckedAt ? `Last checked ${new Intl.DateTimeFormat("en-PH", { timeStyle: "medium" }).format(lastCheckedAt)}` : "Connecting to the municipal feed…";
