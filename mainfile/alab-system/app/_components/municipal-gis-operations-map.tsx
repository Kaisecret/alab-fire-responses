...
export function MunicipalGisOperationsMap() {
  ...
  return <main className="mbfp-ops-root"><style>{styles}</style><section className="mbfp-ops-workspace" aria-labelledby="municipal-gis-heading">
    <header className="mbfp-ops-toolbar"><div><p className="mbfp-ops-eyebrow">Municipal incident history</p><h1 id="municipal-gis-heading" className="mbfp-ops-title">GIS incident map</h1></div><div className="mbfp-ops-tools"><button className="mbfp-ops-refresh" type="button" onClick={() => void refresh(true)} disabled={refreshing}><i className="fa-solid fa-rotate-right" aria-hidden="true" />{refreshing ? "Refreshing map" : "Live refresh"}</button></div></header>
    <div className="mbfp-ops-map-shell"><div ref={mapElement} className="mbfp-ops-map" aria-label="Municipal incident map" />{!mapReady && <div className="mbfp-ops-map-loading" aria-label="Loading municipal incident map" />}{densityEvidence && <aside className="mbfp-ops-density-legend"><strong>Mapped building-density evidence</strong>Red polygons are mapped structures within the incident assessment area.</aside>}{!loading && !error && incidents.length === 0 && <div className="mbfp-ops-empty" role="status"><strong>No incidents have been reported in your assigned municipality</strong><p>The map is centered on {stationName}. New resident alerts appear automatically.</p></div>}</div>
