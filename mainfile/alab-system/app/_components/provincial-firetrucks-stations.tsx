...
const styles = `
  .prov-fleet-tabs { display:inline-flex; gap:.2rem; margin:.35rem 0 0; padding:.28rem; border:1px solid #cbd9e8; border-radius:12px; background:#fff; box-shadow:0 5px 14px rgba(15,23,42,.06); font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  #prov-fleet-panel .prov-trucks { padding:1.25rem 0 0; }
`;
...
export function ProvincialFiretrucksStations() {
  ...
  return <>
    ...
    <div className="prov-fleet-tabs" role="tablist" aria-label="Fire trucks and stations">
      <button type="button" role="tab" id="prov-fleet-tab-trucks" aria-controls="prov-fleet-panel" aria-selected={view === "trucks"} className="prov-fleet-tab" onClick={() => select("trucks")}><i className="fa-solid fa-truck-moving" aria-hidden="true" />Fire trucks</button>
      <button type="button" role="tab" id="prov-fleet-tab-stations" aria-controls="prov-fleet-panel" aria-selected={view === "stations"} className="prov-fleet-tab" onClick={() => select("stations")}><i className="fa-solid fa-building-shield" aria-hidden="true" />Stations</button>
    </div>
    <div id="prov-fleet-panel" role="tabpanel" aria-labelledby={view === "trucks" ? "prov-fleet-tab-trucks" : "prov-fleet-tab-stations"}>
      {view === "trucks"
        ? <ProvincialFireTrucks initialMunicipalityId={searchParams.get("municipalityId") ?? ""} />
