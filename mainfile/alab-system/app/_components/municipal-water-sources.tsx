...
const styles = `
  ...
  .water-dialog-backdrop { position:fixed; inset:0; z-index:1000; display:grid; place-items:center; padding:1rem; background:rgba(15,23,42,.58); }
...
export function MunicipalWaterSources() {
  ...
  const [editForm, setEditForm] = useState({ exactLocation: "", quantity: "1" });
  ...
  function openDetails(source: WaterSource) {
    ...
    setEditForm({ exactLocation: source.exactLocation, quantity: String(source.quantity) });
    setEditError("");
  }
  ...
  async function submitEdit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/municipal-bfp/water-sources", {
        ...
        body: JSON.stringify({ id: selectedSource.id, exactLocation: editForm.exactLocation, quantity: Number(editForm.quantity) }),
      });
      const result = await response.json();
      ...
      setEditForm({ exactLocation: updated.exactLocation, quantity: String(updated.quantity) });
    } catch (error) {
